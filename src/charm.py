...
class SnapStoreProxyCharm(CharmBase):
    ...
    def _on_update_status(self, _):
        if not self._stored.db_connected:
            ...
            return
        try:
            status = check_output(["snap-proxy", "status"])
    ...
    def _on_db_relation_changed(self, event):
        ...
        check_output(["psql", "-x", db_url, "-c", "CREATE EXTENSION \"btree_gist\""])
        check_output(["snap-proxy", "config", "proxy.db.connection={}".format(db_url)])
    ...
    def _set_proxy_domain(self, domain):
        check_output(["snap-proxy", "config", "proxy.domain={}".format(domain)])
